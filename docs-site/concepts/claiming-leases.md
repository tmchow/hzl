...
- **Enables coordination** - `task claim --next` skips claimed tasks
...
```
Agent 1: task claim --next -P proj     → Task #1
Agent 2: task claim --next -P proj     → Task #1
...
```bash
hzl task claim <id> --agent <name> --lease 30   # 30 minute lease
```
...
hzl task steal <id> --if-expired --agent new-agent
```
...
- Transfers ownership to the new agent
...
1. **Always use `--agent`** - Track who's doing the work
2. **Use leases for long work** - Enable stuck detection
3. **Review before stealing** - Check checkpoints first
4. **Use `task claim --next`** - Atomic find-and-claim in one step
